...
export type Page = 'home' | 'blogs' | 'editor';
export type Theme = 'light' | 'dark';

function getPageFromHash(): Page {
  if (window.location.hash.startsWith('#/blog')) return 'blogs';
...
function getAnchorFromHash(): string | undefined {
  const match = window.location.hash.match(/^#\/blog\/[^#]+#+(.+)$/);
  return match?.[1];
}
...
const App: React.FC = () => {
  ...
  const [anchor, setAnchor] = useState<string | undefined>(getAnchorFromHash);
  ...
  const renderPage = () => {
    switch (page) {
      ...
      case 'blogs':
        return <BlogPage theme={theme} initialPostId={postId} initialAnchor={anchor} />;
      case 'editor':
  ...
  return (
    <div className="text-slate-900 dark:text-slate-100 min-h-screen font-sans selection:bg-blue-200 dark:selection:bg-blue-900 bg-white dark:bg-dark transition-colors duration-500 overflow-x-hidden">
      {page !== 'editor' && <Header currentPage={page} setPage={handleSetPage} theme={theme} toggleTheme={toggleTheme} />}
